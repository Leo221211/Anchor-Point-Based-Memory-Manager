...
class HashTable {
    ...
    public MemMapNode getPredKey(BigInteger key) {
        ...
        while(anchorNode != null) {
            // check
            temp = anchorNode;
            BigInteger nextNodeStart = anchorNode.startingAddr.add(anchorNode.size);
            if(nextNodeStart.compareTo(key) == 1) {  // next node's starting address is bigger than the to search address: can break
                // Debug.dbgPrint("Found,address " + key + " is in node with starting address: " + anchorNode.startingAddr);
                break;
            }

            // update
            anchorNode = get(nextNodeStart);
        }
        return temp;
    }
